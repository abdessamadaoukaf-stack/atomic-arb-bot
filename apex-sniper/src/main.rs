...
async fn main() -> Result<()> {
    ...
    let alchemy_http = std::env::var("ALCHEMY_HTTP_URL").expect("Missing ALCHEMY_HTTP_URL");
    ...
    let http_provider = ProviderBuilder::new().on_http(alchemy_http.parse()?);
    ...
    while let Some(block) = block_stream.next().await {
        ...
        tokio::spawn(async move {
            ...
            let _ = evaluate_simulation(block_number, state, pool_list).await;
...
async fn evaluate_simulation(block_number: u64, state: Arc<local_state::LocalAmmState>, pools: Vec<Address>) -> Result<()> {
    for i in 0..pools.len() {
        for j in i+1..pools.len() {
            ...
            // Ensure the two pools trade the exact same pair before comparing
            ...
            if best_p > U256::from(MIN_PROFIT_WEI) {
                ...
                // 1. Generate the Smart Contract Calldata (Claude's Logic)
                ...
                // 2. Generate the Dynamic Priority Fee / Bribe (DeepSearch & Grok's Logic)
                let gas_limit: u64 = 600_000;
                let bribe = best_p * U256::from(3) / U256::from(10); // 30% Bribe
                
                // Safe downcast to u128 to satisfy the Alloy Transaction builder
                let bribe_u128: u128 = bribe.try_into().unwrap_or(u128::MAX);
                let mut max_priority_fee_per_gas: u128 = bribe_u128 / (gas_limit as u128);
                
                // HARD CAP: 100 Gwei (100,000,000,000 wei) to prevent wallet drains
                if max_priority_fee_per_gas > 100_000_000_000 {
                    max_priority_fee_per_gas = 100_000_000_000;
                }

                let msg = format!(
                    "🎯 [SIM] PROFIT: {:.4} ETH | Block: {} \n🛣️ Route: {} -> {} \n🛡️ Gas Tip Bribe: {} Gwei \n📦 Payload: {} bytes ready.", 
                    profit_eth, block_number, pools[i], pools[j], (max_priority_fee_per_gas / 1_000_000_000), calldata.len()
                );
